document.addEventListener("DOMContentLoaded", function () {
  const Cdslapi = "api/CDSLAPI.aspx"; window.Cdslapi = Cdslapi; function getNewsletter() {
    const year = new Date().getFullYear(); fetch("/sourcefiles/newsletter.aspx", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ Year: year, Req: "1" }), }).then((response) => response.text()).then((text) => {
      const data = JSON.parse(text); let output = ""; if (data && Array.isArray(data.NewsletterDetails)) {
        const latestNewsletters = data.NewsletterDetails.slice(0, 2); latestNewsletters.forEach((item) => {
          output += `
                        <div class="Newscls">
                            <div class="issue">
                        ...
                        </div>`}); const container = document.getElementById("NewsletterContainerhome"); if (container) container.innerHTML = output
      }
    }).catch(err => console.error("Newsletter Error:", err))
  }
  window.newsPdf = function (action, filePath) {
    const a = document.createElement('a'); a.href = "/" + filePath; if (action === "Download") { a.download = filePath.split('/').pop() || "document" }
    a.target = "_blank"; document.body.appendChild(a); a.click(); document.body.removeChild(a)
  }; getNewsletter(); const canvas = document.getElementById('captchaCanvas'); if (canvas) {
    const ctx = canvas.getContext('2d'); function generateCaptcha() {
      ctx.clearRect(0, 0, canvas.width, canvas.height); const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789'; let captchaText = ''; const length = 5 + Math.floor(Math.random() * 2); for (let i = 0; i < length; i++) { captchaText += chars.charAt(Math.floor(Math.random() * chars.length)) }
